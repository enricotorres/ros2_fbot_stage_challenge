...
class BehavioralNavigatorNode(Node):
    def __init__(self):
        ...
        self.odom_subscription = self.create_subscription(Odometry, "/ground_truth", self.ground_truth_callback, 10)
        self.scan_subscription = self.create_subscription(LaserScan, "base_scan", self.base_scan_callback, 10)
    ...
    def update_goal_state(self):
        ...
        self.angle_to_target = self.normalize_angle(np.rad2deg(np.arctan2(delta_y, delta_x)))
        self.shortest_diff = self.normalize_angle(self.angle_to_target - self.current_yaw)
    ...
    def update_behavioral_state(self, is_obstacle_in_front, is_path_clear):
        ...
        if self.modo == "objetivo":
            if is_obstacle_in_front and self.distance > self.safe_zone:
                self.modo = "desviando"
                self.get_logger().info(f"MODO: DESVIANDO   (Obstáculo à frente!)")
        
        elif self.modo == "desviando":
            if not is_obstacle_in_front:
                self.modo = "contornando"
                self.get_logger().info(f"MODO: CONTORNANDO (Frente livre, andando reto)")
        
        elif self.modo == "contornando":
            if is_obstacle_in_front:
                self.modo = "desviando"
                self.get_logger().info(f"MODO: DESVIANDO   (Bateu em obstáculo, virando)")
                ...
                if self.hysteresis_count >= self.hysteresis_threshold:
                    self.modo = "objetivo"
                    self.hysteresis_count = 0
                    self.get_logger().info(f"MODO: OBJETIVO    (Caminho livre!)")
            else:
                self.hysteresis_count = 0
    ...
    def generate_velocity_command(self):
        ...
        if self.modo == "objetivo":
            if abs(self.shortest_diff) > self.angular_tolerance:
                cmd.linear.x = 0.0
                cmd.angular.z = self.angular_speed if self.shortest_diff > 0 else -self.angular_speed
            else:
    ...
    def detect_obstacle(self, distance_threshold, frontal_angle_deg=30.0):
        ...
        valid = [r for r in frontal_scan if self.scan.range_min <= r <= self.scan.range_max]
    ...
    def is_goal_path_clear(self, safety_dist=5.0):
        ...
        center_idx = int((relative_angle_rad - self.scan.angle_min) / self.scan.angle_increment)
        window_deg = 10.0 
